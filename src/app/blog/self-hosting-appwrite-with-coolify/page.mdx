...
![Cover Image](/images/blog/self-hosting-appwrite-with-coolify/cover.png)
...
4. ![Create Droplet](/images/blog/self-hosting-appwrite-with-coolify/create-droplet.png)

5. For OS, select **Ubuntu** and select the latest version.
6. Choose a Droplet plan based on your expected workload.
7. Add block storage if you expect to store large files _(optional but recommended for production)_.
8. Enable backups for production environments _(optional but recommended)_.
...
Create a non-root user with sudo privileges _(optional but recommended)_:
...
While accessing the console, you might run into the error `ERR_TOO_MANY_REDIRECTS` _(Site redirected you too many times)_. There can be a couple of issues here:
...
<OriginalPost
  text="Read the original post on Appwrite's blog"
  link="https://appwrite.io/blog/post/self-hosting-appwrite-with-coolify"
/>
